...
app.post('/places', (req, res) => {
  ...
  const availableLocations = AVAILABLE_LOCATIONS.filter(loc => !INTERESTING_LOCATIONS.includes(loc));
  const suggestedLocations = getSuggestedLocations()
  res.send(
  `
    ...
    <ul id="available-locations" class="locations" hx-swap-oob="true">
      ${availableLocations.map((location) => renderLocation(location , true)).join('')}
    </ul>
  `
  );
});
...
app.get("/suggested-locations" , (req , res) => {
  const suggestedLocations = getSuggestedLocations();
  res.send(
    suggestedLocations.map((loc) => renderLocation(loc)).join('')
  )
})
